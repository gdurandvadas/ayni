...
- Status: validated
- Changed responsibilities: dependency resolution is locked across quality, documentation, release, and example Docker workflows; PR CI owns real-tool matrix validation for Go, Node, Python, and Kotlin; the standard-library validator owns typed artifact and policy-result validation; Kotlin uses pinned stable Gradle 8.10.2.
- Changed paths: `.github/workflows/ayni.yml`, `.github/workflows/docs.yml`, `.github/workflows/release.yml`, `examples/{rust,go,node,python,kotlin}/Dockerfile`, `scripts/ci/validate_example_artifact.py`, `scripts/ci/test_validate_example_artifact.py`. `Cargo.lock` had no Milestone 6 diff because approved 0.8 metadata was committed early.
- Outcomes: all eight workspace lock entries are 0.8.0 with no dependency churn; all dependency-resolving Cargo commands in quality/docs/release and the five example Dockerfiles use `--locked`. PR CI checks out the CLI, applies/checks catalogs, analyzes mono fixtures, always validates typed artifacts after policy exit, and removes ephemeral `.ayni`. The standard-library validator enforces schema 0.3.0, exact target/kind row sets, completion/account reconciliation, unique rows, no command failures, finite data, and typed policy-only failures; 15 tests include the lexical relative repository-root regression. Kotlin uses pinned stable Gradle 8.10.2; no fixture changes.
