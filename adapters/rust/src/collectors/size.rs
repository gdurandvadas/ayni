...
pub fn collect(context: &RunContext) -> Result<SignalRow, String> {
    let size_map = context.policy.size_rules_for(Language::Rust);
    if size_map.is_empty() {
        return Err(String::from(
            "missing size config: add [rust.size] with at least one glob entry to .ayni.toml",
        ));
    }

    let excluded = &["target", ".git", ".ayni"];
