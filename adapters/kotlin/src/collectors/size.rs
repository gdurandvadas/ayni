...
pub fn collect(context: &RunContext) -> Result<SignalRow, String> {
    let size_map = context.policy.size_rules_for(Language::Kotlin);
    if size_map.is_empty() {
        return Err(String::from(
            "missing size config: add [kotlin.size] with at least one glob entry to .ayni.toml",
        ));
    }
        ...
        .filter_entry(|entry| !is_excluded_dir(entry.path()))
    {
        ...
        let rel_for_match = to_repo_relative_path(&context.workdir, entry.path());
