...
pub fn ensure_gradle_plugins(root: &Path) -> Result<(), String> {
    ...
    let use_kover = !repo_uses_jacoco(root)?;
    let updated = if path.file_name().and_then(|value| value.to_str()) == Some("build.gradle.kts") {
        ensure_plugins(&content, Dsl::Kotlin, use_kover)?
    } else {
        ensure_plugins(&content, Dsl::Groovy, use_kover)?
    };
    if updated != content {
...
fn ensure_plugins(content: &str, dsl: Dsl, use_kover: bool) -> Result<String, String> {
    ...
    for (id, version) in PLUGINS {
        if *id == "org.jetbrains.kotlinx.kover" && !use_kover {
            continue;
        }
...
fn repo_uses_jacoco(root: &Path) -> Result<bool, String> {
    let build_files = WalkDir::new(root)
        .into_iter()
        .filter_entry(|entry| {
            if entry.depth() == 0 {
                return true;
    ...
    for path in build_files {
        let content = fs::read_to_string(&path)
            .map_err(|error| format!("failed to read {}: {error}", path.display()))?;
        if gradle_content_uses_jacoco(&content) {
            return Ok(true);
        }
    }
    Ok(false)
...
fn gradle_content_uses_jacoco(content: &str) -> bool {
    content.contains("jacoco")
        || content.contains("JacocoReport")
        || content.contains("jacocoTestReport")
}

#[cfg(test)]
mod tests {
    use super::{ensure_gradle_plugins, gradle_content_uses_jacoco};
    use std::fs;
    ...
    #[test]
    fn skips_kover_when_repo_already_uses_jacoco() {
        let dir = TempDir::new().expect("tempdir");
        let path = dir.path().join("build.gradle.kts");
        fs::write(
            &path,
        ...
        .expect("build");
        fs::create_dir_all(dir.path().join("build-logic")).expect("build-logic");
        fs::write(
            dir.path().join("build-logic/build.gradle.kts"),
            "plugins { id(\"jacoco\") }\n",
        )
        .expect("jacoco marker");

        ensure_gradle_plugins(dir.path()).expect("plugins");
        let updated = fs::read_to_string(path).expect("updated");

        assert!(!updated.contains("org.jetbrains.kotlinx.kover"));
        assert!(updated.contains("io.gitlab.arturbosch.detekt"));
    }

    #[test]
    fn detects_jacoco_markers_in_gradle_content() {
        assert!(gradle_content_uses_jacoco("plugins { id(\"jacoco\") }\n"));
        assert!(gradle_content_uses_jacoco(
            "tasks.withType<JacocoReport> {}\n"
        ));
        assert!(!gradle_content_uses_jacoco("plugins { kotlin(\"jvm\") }\n"));
    }
